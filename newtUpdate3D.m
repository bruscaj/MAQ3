function [deltaU] = newtUpdate3D(NMatSDD,CMatSDD,Dvvs,weight,uNewt,F,epsilon)
...
spmat = spdiags((SDDMat(NMatSDD,CMatSDD,uNewt,vCount,epsilon).^(-3/2)*weight),0,intLength,intLength)^3;
...
for i = 1:vCount
    ...
    D = weight(i)*spdiags(Uvv.^(-5/2),0,intLength,intLength);
...
jacobian = 12*pi^2*(spmat\jacobian) + reg;
% jacobian = correction*jacobian+reg;
% Combine all terms of jacobian

% 4*pi^2*((SDDMat(NMatSDD,CMatSDD,uNewt,vCount,epsilon).^(-3/2))*weight).^(-2)+min(min(SDDMat(NMatSDD,CMatSDD,uNewt,vCount,-Inf),epsilon),[],2);
resid = sparse(4*pi^2*((SDDMat(NMatSDD,CMatSDD,uNewt,vCount,epsilon).^(-3/2))*weight).^(-2)+min(min(SDDMat(NMatSDD,CMatSDD,uNewt,vCount,-Inf),epsilon),[],2)-F);
% Could be neater...
